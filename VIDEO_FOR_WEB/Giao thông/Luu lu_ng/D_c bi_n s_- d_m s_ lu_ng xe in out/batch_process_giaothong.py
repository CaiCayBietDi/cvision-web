...
def process_image_pair(orig_path, marked_path, out_path):
    ...
    with open(marked_path, 'rb') as f:
        marked = cv2.imdecode(np.frombuffer(f.read(), np.uint8), cv2.IMREAD_COLOR)
    ...
    _, diff_mask = cv2.threshold(gray_diff, 20, 255, cv2.THRESH_BINARY)
...
dir_path = r"F:\CVISION\antigravity project\web\Giao thông (1)\Lưu lượng\Đọc biển số- đếm số lượng xe in out"
out_dir = os.path.join(dir_path, "modify")

if not os.path.exists(out_dir):
    os.makedirs(out_dir)

print("Đang tìm và xử lý các ảnh có đuôi _m.jpg trong thư mục...")
